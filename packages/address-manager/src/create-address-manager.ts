...
export function createAddressManager(env: AddressManagerEnv) {
  const discoveryUrls = resolveDiscoveryUrls(env.ADDRESS_MANAGER_URL, env.ADDRESS_MANAGER_URLS);
  const wsSubscribedServices = resolveWsSubscribedServices(env.WS_SUBSCRIBED_SERVICES);
...
/**
 * Resolve the list of discovery URLs.
 *
 * Precedence:
 * 1. ADDRESS_MANAGER_URLS (JSON array, e.g. '["https://ds-us:3000","https://ds-eu:3000"]')
 * 2. ADDRESS_MANAGER_URL (single URL, legacy)
 */
/**
