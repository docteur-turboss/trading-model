...
export class WebSocketClient {
	...
	constructor(options: WebSocketClientOptions) {
		...
		this._reconnectHandler = new DefaultWsReconnector({ maxAttempts: options.maxReconnectAttempts ?? 10, config: { baseDelayMs: options.reconnectIntervalMs ?? 5000, maxDelayMs: options.reconnectIntervalMs ?? 5000, jitterMs: 0 }, onReconnect: () => this.connect() });
	...
	connect(): void { if (!this._connection.isConnected) this._connection.connect(); }
	private _onOpen(): void {
		this._reconnectHandler.reset();
		logger.info("WebSocket connected to discovery server", { url: this._connection.url });
		this.send(DiscoveryWsMessageType.Subscribe, { services: this._subscribedServices });
	}
	private _onMessage(data: WebSocket.Data): void {
		try { this._eventHandler(JSON.parse(data.toString()) as WsMessage); } catch (err) { logger.warn("Failed to parse WebSocket message", { data: data.toString(), err: normalizeError(err) }); }
	}
	private _onClose(): void {
		if (this._connection.lastCloseCode === 4001) { this._authFailureHandler(); return; }
		this._reconnectHandler.scheduleReconnect();
	}
	private _onError(error: Error): void { logger.error("WebSocket error", { error: normalizeError(error) }); }
	...
	send(typeOrData: WsMessageType | unknown, payload?: Record<string, unknown>): boolean {
		if (payload !== undefined) return this._connection.send(JSON.stringify({ type: typeOrData as WsMessageType, payload }));
		return this._connection.send(JSON.stringify(typeOrData));
	}
	disconnect(): void { this._reconnectHandler.cancel(); this._connection.disconnect(); }
	get isConnected(): boolean { return this._connection.isConnected; }
	getReconnectAttempts(): number { return this._reconnectHandler.attempt; }
	updateToken(token: string): void { this._connection.updateToken(token); }
	sendHeartbeat(identity: ServiceIdentity): boolean {
		return this.send(DiscoveryWsMessageType.Heartbeat, { serviceName: identity.serviceName, instanceId: identity.instanceId });
	}
}
