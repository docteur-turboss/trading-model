...
import { WsConnection } from "../../../infrastructure/client/ws-connection";
...
export class WebSocketClient {
	...
	constructor(options: WebSocketClientOptions) {
		...
		this._connection.onMessage = (data) =>
			this._onMessage(data as WebSocket.Data);
		this._connection.onCloseHandler = (code) => this._onClose(code);
	...
	private _onOpen(): void {
		...
		this.send(DiscoveryWsMessageType.Subscribe, {
			services: this._subscribedServices,
		});
	}
	...
	private _onClose(code?: number): void {
		if (code === WS_CLOSE_CODES.AUTH_FAILURE) {
