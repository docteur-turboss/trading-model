...
import { computeCpuPercent, SystemMetrics } from '../../src/monitoring/system-metrics';
    ...
    test('should handle totalMem of 0 in usedPercent calculation', () => {
      jest.isolateModules(() => {
        jest.doMock('os', () => ({
          totalmem: () => 0,
          freemem: () => 0,
          cpus: () => [{ times: { idle: 100, user: 0, nice: 0, sys: 0, irq: 0 } }],
          loadavg: () => [0, 0, 0],
          uptime: () => 0,
        }));
        const { SystemMetrics: MockedMetrics } = require('../../src/monitoring/system-metrics');
