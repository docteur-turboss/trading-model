...
describe('AddressManagerClient Multi-URL', () => {
  ...
  function makeConfig(overrides?: Partial<AddressManagerConfig>): AddressManagerConfig {
    return {
  ...
  function createMockHttpClient(): jest.Mocked<HttpClient> {
    return {
      post: jest.fn<(url: string) => Promise<unknown>>().mockResolvedValue(undefined),
      get: jest.fn<(url: string) => Promise<unknown>>().mockResolvedValue(undefined),
      delete: jest.fn<(url: string) => Promise<unknown>>().mockResolvedValue(undefined),
  ...
  test('should try primary URL first and succeed', async () => {
    ...
    httpClient.post.mockResolvedValueOnce({ token: 'new-token', instanceId: 'test-instance' });
  ...
  test('should fall back to secondary URL when primary fails', async () => {
    ...
    httpClient.post
      ...
      .mockResolvedValueOnce({ token: 'new-token', instanceId: 'test-instance' });
