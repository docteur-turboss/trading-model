...
export interface KeyVault {
  ...
  write(keyPath: string, keyPair: KeyPair, options?: { mode?: number }): Promise<void>;
  exists(keyPath: string): Promise<boolean>;
}

export class FileKeyVault implements KeyVault {
  async generate(algorithm: KeyAlgorithm = KeyAlgorithm.EC_P384): Promise<KeyPairWithId> {
  ...
  async write(keyPath: string, keyPair: KeyPair, options?: { mode?: number }): Promise<void> {
    const dir = keyPath.substring(0, keyPath.lastIndexOf('/'));
    await mkdir(dir, { recursive: true });
    await writeFile(keyPath, keyPair.privateKey, { mode: options?.mode ?? 0o600 });
  }
  ...
  async exists(keyPath: string): Promise<boolean> {
    try {
      ...
      logger.warn('Key file existence check failed', { keyPath, err: normalizeError(err) });
      return false;
    }
  }
}
