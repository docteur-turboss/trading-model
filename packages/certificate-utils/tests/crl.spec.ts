...
import { RevokedCertificate } from '../src/types';

function makeRevokedEntry(overrides?: Partial<RevokedCertificate>): RevokedCertificate {
  return {
    serialNumber: 'SN-001',
...
describe('createCrl', () => {
  ...
  it('should use default TTL of 7 days', () => {
    const crl = createCrl([]);
    const expectedNextUpdate = crl.lastUpdate.getTime() + 7 * 24 * 60 * 60 * 1000;
...
describe('isRevoked', () => {
  ...
  it('should return true for a recent revocation entry', () => {
    ...
    const crl = createCrl([makeRevokedEntry({ serialNumber: 'SN-RECENT', revokedAt: recent })]);
