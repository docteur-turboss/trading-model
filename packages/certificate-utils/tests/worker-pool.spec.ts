...
describe('WorkerPool', () => {
  ...
  it('should reject when worker sends error', async () => {
    ...
    const call = fakeWorkers[0].postMessage.mock.calls[0][0] as { id: string };
    triggerMessage(fakeWorkers[0], { id: call.id, success: false, error: 'keygen failed' });
  ...
  it('should dispatch queued task when worker becomes idle', async () => {
    ...
    const call2 = fakeWorkers[0].postMessage.mock.calls[1][0] as { id: string; type: string };
  ...
  it('should replace worker on error', () => {
    ...
    const initialCount = fakeWorkers.length;
    triggerError(fakeWorkers[0]);

    expect(fakeWorkers.length).toBe(initialCount + 1);
    pool.terminate();
  });
  ...
  it('should not replace worker on zero exit', () => {
    ...
    const initialCount = fakeWorkers.length;
    triggerExit(fakeWorkers[0], 0);

    expect(fakeWorkers.length).toBe(initialCount);
    pool.terminate();
  });
  ...
  it('should not replace a worker that was already removed', () => {
    ...
    const oldWorker = fakeWorkers[0];
    triggerError(oldWorker);

    const countAfterFirstError = fakeWorkers.length;
    triggerError(oldWorker);

    expect(fakeWorkers.length).toBe(countAfterFirstError);
    pool.terminate();
  });
  ...
  it('should handle unknown message id', () => {
    const pool = new WorkerPool({ size: 1, workerScript: '/fake/worker.js' });
    pool.execute('generateKeyPair', { algorithm: 'ec' }).catch(() => {});
    triggerMessage(fakeWorkers[0], { id: 'unknown-id', success: true, data: null });
    pool.terminate();
  });
  ...
  it('should use default error message when worker sends error without message', async () => {
    ...
    const call = fakeWorkers[0].postMessage.mock.calls[0][0] as { id: string };
    triggerMessage(fakeWorkers[0], { id: call.id, success: false });
  ...
  it('should break processQueue when all workers busy with more queued tasks', () => {
    ...
    const call1 = fakeWorkers[0].postMessage.mock.calls[0][0] as { id: string };
    triggerMessage(fakeWorkers[0], { id: call1.id, success: true, data: null });
    ...
    triggerMessage(fakeWorkers[0], { id: call2.id, success: true, data: null });

    expect(pool.pending).toBe(0);
    expect(fakeWorkers[0].postMessage).toHaveBeenCalledTimes(3);
    pool.terminate();
  });
});
