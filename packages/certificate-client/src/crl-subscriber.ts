...
export async function subscribeToCertificateEvents(
  ...
  const cleanupRevoked = EventManager.on(EnumEventMessage.certificateRevoked, (payload: unknown) => {
  ...
  await messageManager.intents([EnumEventMessage.certificateRevoked, EnumEventMessage.caKeyRotated]);
