...
const serviceCircuitBreakers = new Map<string, ServiceCircuitBreakerEntry>();
...
function getServiceEntry(serviceName: string): ServiceCircuitBreakerEntry {
  let entry = serviceCircuitBreakers.get(serviceName);
  if (!entry) {
    entry = { failures: 0, state: 'closed', lastFailureTime: 0 };
    serviceCircuitBreakers.set(serviceName, entry);
  }
  return entry;
}
 ...
 */
function recordServiceFailure(serviceName: string, instanceCount?: number): void {
  const entry = getServiceEntry(serviceName);
  entry.failures++;
  entry.lastFailureTime = Date.now();
...
export function isServiceCircuitOpen(serviceName: string): boolean {
  const entry = serviceCircuitBreakers.get(serviceName);
...
export class HttpClient {
  ...
  private readonly tlsPaths?: { ca?: string; cert?: string; key?: string };
  private tlsLoadPromise: Promise<void> | null = null;
   ...
   */
  private async ensureTlsLoaded(): Promise<void> {
    if (this.tlsLoaded || !this.tlsPaths) return;
    if (this.tlsLoadPromise) return this.tlsLoadPromise;

    this.tlsLoadPromise = (async () => {
      if (!this.tlsPaths) {
        this.tlsLoaded = true;
        return;
      }
      const { ca, cert, key } = this.tlsPaths;
      if (ca) this.ca = await readTlsFile(ca, 'CA certificate');
      if (cert) this.cert = await readTlsFile(cert, 'client certificate');
      if (key) this.key = await readTlsFile(key, 'client key');
      this.tlsLoaded = true;
    })();

    return this.tlsLoadPromise;
  }
  ...
  private async request<T>(
    ...
    if (this.tlsPaths && !this.tlsLoaded) {
      await this.ensureTlsLoaded();
    }
    ...
    if (serviceName) checkServiceCircuit(serviceName);
    ...
    for (let attempt = 0; attempt <= retryCount; attempt++) {
      try {
        ...
        if (serviceName) recordServiceFailure(serviceName, options?.serviceInstanceCount);
        throw lastError;
      }
    }
 ...
 */
export function computeAdaptiveTimeout(baseMs: number, ewmLatencyMs?: number): number {
