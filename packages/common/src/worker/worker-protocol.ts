...
export class WorkerProtocol implements IWorkerProtocol {
	...
	constructor(server: https.Server, private readonly _workerRegistry: WorkerRegistry, private readonly _onWorkerDisconnect: (workerId: string) => void) {
	...
	private _onWsMessage(message: WorkerIncomingMessage, ws: WebSocket): void { this._handlers[message.type]?.(message, ws); }
