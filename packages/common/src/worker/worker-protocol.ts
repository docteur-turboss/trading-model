...
export class WorkerProtocol implements IWorkerProtocol {
	...
	constructor(
		...
		this._wsManager = new WorkerWsManager(server, (msg, ws) => this._onWsMessage(msg, ws), (ws) => this._onWsClose(ws));
		this._handlers = this._buildHandlerMap();
	}

	private _buildHandlerMap(): Partial<Record<WorkerIncomingMessage["type"], (message: WorkerIncomingMessage, ws?: WebSocket) => void>> {
		return {
	...
	private _handleRegister(message: WorkerIncomingMessage & { type: "register" }, ws: WebSocket): void {
		this._workerRegistry.register(message.workerId, {
			workerId: message.workerId, address: message.address, port: message.port,
			capabilities: message.capabilities, maxConcurrency: message.maxConcurrency, currentLoad: 0,
		});
	...
	private _handleHeartbeat(message: WorkerIncomingMessage & { type: "heartbeat" }): void {
	...
	private _handleDisconnect(message: WorkerIncomingMessage & { type: "disconnect" }): void {
		...
		logger.info("Worker disconnected", { context: { workerId: message.workerId, reason: message.reason } });
	}

	sendToWorker(workerId: string, message: SchedulerOutgoingMessage): void { this._wsManager.sendToWorker(workerId, message); }
	sendDrain(workerId: string): void { this._wsManager.sendDrain(workerId); }
	broadcastDrain(): void { this._wsManager.broadcastDrain(); }
	close(): void { this._wsManager.close(); }
}
