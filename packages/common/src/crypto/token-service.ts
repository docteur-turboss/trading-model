...
export function generateInstanceToken(instanceId: string, signingSecret: string): string {
...
export function validInstanceToken(
  ...
  if (parts.length === 4) {
    ...
    const ts = parseInt(Buffer.from(timestampB64, 'base64url').toString('utf8'), 10);
    const now = Date.now();
    if (isNaN(ts)) return false;
    if (now - ts > maxAge + clockSkewTolerance) return false;
    if (ts - now > clockSkewTolerance) return false;
  }

  if (!verifyHmac(encodedId, payloadParts, signature, signingSecret)) return false;
  ...
  if (options?.allowSlidingExpiry && storedToken) {
    ...
    if (storedParts.length === 3 || storedParts.length === 4) {
      ...
      if (verifyHmac(storedEncodedId, storedPayloadParts, storedSignature, signingSecret)) {
        return true;
      }
    }
  }
...
export function generateInstanceId(serviceName: string, address: string, port: number): string {
