...
describe("DelayRange", () => {
	describe("constructor", () => {
		it("should create a DelayRange with valid parameters", () => {
			const dr = new DelayRange({
				baseMs: DurationMs.of(100),
				maxMs: DurationMs.of(5000),
			});
		...
		it("should throw when baseMs <= 0", () => {
			expect(
				() =>
					new DelayRange({
						baseMs: DurationMs.of(0),
						maxMs: DurationMs.of(100),
					})
			).toThrow(RangeError);
			expect(
				() =>
					new DelayRange({
						baseMs: DurationMs.of(-1),
						maxMs: DurationMs.of(100),
					})
			).toThrow(RangeError);
		});
