...
import { IJobRepository } from '../../../src/recovery/job-repository.interface';
import { IJobQueue } from '../../../src/recovery/job-queue.interface';
import { WorkerRegistry } from '../../../src/worker/worker-registry';
import { Job } from '../../../src/contracts/recovery.types';
...
describe('OrphanDetector (shared)', () => {
  ...
  describe('detection cycle', () => {
    ...
    it('should update status to orphaned and reallocate stale jobs', async () => {
      ...
      expect(mockRepository.updateStatus).toHaveBeenCalledWith('orphan-1', 'orphaned');
    });
