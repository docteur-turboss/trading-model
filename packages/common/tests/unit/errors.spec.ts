...
describe("AppError", () => {
	...
	it("should support reason via options", () => {
		const error = new AppError("custom reason", ErrorCodes.NACK_ERROR, {
	...
	it("should store cause with NACK_ERROR", () => {
		const cause = new Error("root");
		const error = new AppError("reason", ErrorCodes.NACK_ERROR, {
			reason: "reason",
			cause,
		});
		expect(error.cause).toBe(cause);
	...
	it("should store cause with DEAD_LETTER_ERROR", () => {
		const cause = new Error("root");
		const error = new AppError("reason", ErrorCodes.DEAD_LETTER_ERROR, {
			reason: "reason",
			cause,
		});
		expect(error.cause).toBe(cause);
