...
import { WorkerClient } from "@trading-model/jobs/application/services/worker-client";

function getWs(): any {
	const entry = MOCK_WEB_SOCKET.mock.results[0];
...
function getCallbacks(ws: any) {
	...
	return {
		...
		message: onCalls.find((c: any) => c[0] === "message")?.[1] as (
			data: string
		) => void,
		close: onCalls.find((c: any) => c[0] === "close")?.[1] as () => void,
		error: onCalls.find((c: any) => c[0] === "error")?.[1] as (
			err: Error
		) => void,
	};
}
...
describe("WorkerClient", () => {
	...
	describe("connect", () => {
		it("should create a WebSocket connection to the server URL", async () => {
			...
			expect(MOCK_WEB_SOCKET).toHaveBeenCalledWith("wss://scheduler:3000");
	...
	describe("message handling", () => {
		...
		it("should emit unknown for unrecognized message types", async () => {
			...
			getCallbacks(ws).message!(
				JSON.stringify({ type: "unknown-type", foo: "bar" })
			);
