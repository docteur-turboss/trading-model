...
| ------------------ | ------------------------------------------------ | ----------- | --------------------------- | ---------- |
| MongoDB            | audit-logger, dlq-service                      | High        | 1 hour                      | 4 hours    |
| MySQL              | financial-scraper, SPIRE Server (datastore)    | High        | 1 hour                      | 4 hours    |
...
| SPIRE datastore    | SPIRE Server (MySQL `spire`)                     | Critical    | 24 hours                    | 4 hours    |
...
| --------------------------------------- | ------------------------------------------------ | ------------------------------- |
| MongoDB (dlq-service, audit-logger)     | `mongodump --gzip --oplog --archive`             | Point-in-time consistent        |
| MySQL (financial-scraper + SPIRE `spire`) | `mysqldump --single-transaction --all-databases` | Transaction-consistent snapshot |
...
| Secrets leak           | Security breach  | Rotate all secrets, rotate SPIRE trust bundle / SVIDs  |
...
3. Restore SPIRE datastore from backup
4. Restore databases from latest backup (MongoDB → MySQL incl. SPIRE `spire` → Redis)
5. Start core infrastructure: Redis → MongoDB → MySQL → SPIRE Server + Agent
6. Start services in dependency order:
   a. discovery-server
   b. message-manager
   c. api-gateway
   d. audit-logger, dlq-service
   e. financial-scraper, trader-trainer
   f. admin-interface
...
# Check core service health (mTLS via SPIRE SVIDs, run inside a service container)
for svc in discovery-server message-manager api-gateway; do
  docker compose exec -T "$svc" sh -c \
    'curl -sk --cert /run/spire/svid/svid.pem --key /run/spire/svid/svid_key.pem https://localhost:3000/ping'
done
```
