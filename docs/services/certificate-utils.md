...
`@trading-model/certificate-utils` is consumed by the **certificate-authority** service (`docs/services/certificate-authority.md`) to issue, validate, and manage TLS certificates for the microservice mesh. It uses a **custom certificate format** (JSON + base64 PEM, not RFC 5280) optimised for the platform's internal mTLS needs.
