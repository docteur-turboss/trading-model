...
describe('API Gateway E2E', () => {
  ...
  it('should reject proxy requests without auth', async () => {
    const { status } = await fetchUrl(`${GATEWAY}/v1/discovery-server/services`);
    expect(status).toBe(401);
  });

  it('should proxy to discovery-server with valid auth', async () => {
    const { status, body } = await fetchUrl(`${GATEWAY}/v1/discovery-server/services`, {
  ...
  it('should proxy health check through gateway', async () => {
    const { status, body } = await fetchUrl(`${GATEWAY}/v1/discovery-server/health`, {
      headers: { 'x-api-key': API_KEY },
    });
    expect(status).toBe(200);
    const parsed = JSON.parse(body);
  ...
  it('should return 400 for invalid routes', async () => {
    const { status } = await fetchUrl(`${GATEWAY}/v1/nonexistent-service/ping`, {
      headers: { 'x-api-key': API_KEY },
    });
