...
const topicPattern = /^[a-z]+\.[a-z]+\.[a-z]+$/;
...
const TickerPayload = MarketDataPayload.extend({
  open: z.number(),
  high: z.number(),
  low: z.number(),
  last: z.number(),
...
const CandlePayload = MarketDataPayload.extend({
  open: z.number(),
  high: z.number(),
  low: z.number(),
  close: z.number(),
...
const TradePayload = MarketDataPayload.extend({
  price: z.number(),
  quantity: z.number(),
...
const contracts: Contract[] = [
  ...
  // ── discovery-server contracts ─────────────────────────────────────────
  {
    ...
    consumers: ['message-manager', 'certificate-authority', 'api-gateway', 'dlq-service'],
  ...
  {
    producer: 'message-manager',
    consumers: ['financial-scraper', 'trader-trainer', 'api-gateway', 'audit-logger'],
  ...
  for (const contract of contracts) {
    describe(`${contract.producer} → ${contract.consumers.join(', ')}`, () => {
      it(`[${contract.version}] topic "${contract.topic}" should match pattern`, () => {
        expect(contract.topic).toMatch(topicPattern);
      });
      ...
      it(`[${contract.version}] schema should accept minimal valid payload`, () => {
        const result = contract.payloadSchema.safeParse(contract.samplePayload());
        expect(result.success).toBe(true);
      });
