...
import addressManager from "cash-lib/adress-manager/index";
...
addressManager.init(app, {
  serviceName: ServiceInstanceName.MessageDeliveryService, 
  addressManagerUrl: process.env.ADDRESS_MANAGER_URL || "http://localhost:8443",
  cacheTtlMs: 30000,
 ...
 */
server.listen(PORT, () => {
  logger.info(`Message Manager Service listening on port ${PORT}`);
});