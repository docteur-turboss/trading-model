...
export const NORMALIZERS: Record<NormalisationType, Normalizer> = {
  ...
  'logarithmic-normalization': {
    normalize: (data: Float32Array, len: number) => {
      for (let i = 0; i < len; ++i) {
        ...
        data[i] = x < 0 ? -Math.log(1 - x) : Math.log(1 + x);
      }
  ...
  'robust-scaling': {
    normalize: (data: Float32Array, len: number) => {
      ...
      const median = n & 1 ? sorted[n >> 1] : (sorted[(n >> 1) - 1] + sorted[n >> 1]) * 0.5;
  ...
  'z-score': {
    normalize: (data: Float32Array, len: number) => {
      ...
      for (const x of data) variance += (x - mean) ** 2;
