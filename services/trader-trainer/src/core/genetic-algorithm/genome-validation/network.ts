...
export function validateLayer(
	...
	if (!VALID_ACTIVATIONS.has(layer.activation)) {
		err({ ...ctx, path: `${ctx.path}.activation` }, "unknown activation type", layer.activation);
	}
	if (!VALID_CONNECTION_TYPES.has(layer.connectionType)) {
		err({ ...ctx, path: `${ctx.path}.connectionType` }, "unknown connection type", layer.connectionType);
	}
...
export function validateNetwork(
	...
	if (!Array.isArray(network.hiddenLayers) || network.hiddenLayers.length === 0) {
		err({ ...ctx, path: "network.hiddenLayers" }, "must be a non-empty array", network.hiddenLayers);
	} else {
	...
	if (!VALID_NORM_TYPES.has(network.normalization)) {
		err({ ...ctx, path: "network.normalization" }, "unknown normalization type", network.normalization);
	}
}
