...
describe('EvolutionEngine', () => {
  ...
  describe('selectParent', () => {
    ...
    it('should pick a candidate with higher fitness over a lower one', () => {
      ...
      const controlledRng = () => {
        callCount++;
        if (callCount === 1) return 0; // best = index 0 (fitness -100)
        if (callCount === 2) return 0.3334; // cand = index 1 (fitness 100)
        return 0;
      };
    ...
    it('should handle all undefined fitnesses', () => {
      ...
      const controlledRng = () => {
        callCount++;
        if (callCount === 1) return 0.0; // best = index 0 (id='a')
        if (callCount === 2) return 0.5; // cand = index 1 (id='b'), but -Inf > -Inf is false
        if (callCount === 3) return 0.99; // cand = index 2 (id='c'), same
        return 0;
      };
      const parent = selectParent(population, 'tournament', controlledRng);
