...
import type { MutationGenome, LayerGenome } from '../../../src/core/genetic-algorithm/genome-types';

describe('Mutation - adaptSigma', () => {
  const rng = () => 0.5;

  function makeMutationGenome(overrides?: Partial<MutationGenome>): MutationGenome {
    return {
  ...
  test('self_adaptive returns positive value', () => {
    const m = makeMutationGenome({ adaptation: 'self_adaptive', sigma: 0.5, selfSigma: 0.3 });
    const result = adaptSigma(m, rng);
  ...
  test('unknown adaptation returns sigma', () => {
    const m = makeMutationGenome({ adaptation: 'unknown_value' as any, sigma: 0.5 });
    expect(adaptSigma(m, rng)).toBe(0.5);
  });
});
...
describe('Mutation - mutateGenome', () => {
  ...
  test('should trigger addNeuronRate structural mutation', () => {
    ...
    for (let i = 0; i < origNeurons.length; i++) {
      if (mutated.network.hiddenLayers[i]!.neurons !== origNeurons[i]) changed = true;
    }
  ...
  test('should trigger horizon nStepReturn +1 and frameSkip -1 branches', () => {
    ...
    const rng = () => {
      idx++;
      if (idx === 15) return 0.05; // nStepReturn outer  < 0.1  → true
      if (idx === 16) return 0.05; // nStepReturn inner  < 0.5  → true  → +1
      if (idx === 17) return 0.05; // frameSkip outer   < 0.1  → true
      if (idx === 18) return 0.6; // frameSkip inner  >= 0.5  → false → -1
      return 0.5;
    };
    const mutated = mutateGenome(genome, rng);
