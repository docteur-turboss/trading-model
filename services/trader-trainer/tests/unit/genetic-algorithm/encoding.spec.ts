...
describe('encoding', () => {
  describe('encodeGenome', () => {
    ...
    test('should encode gamma at index 0', () => {
      ...
      const vec = encodeGenome(g);
      expect(vec[0]).toBeCloseTo(g.rl.gamma, 4);
    });
    ...
    test('should encode neuron count at layer base normalised by 512', () => {
      ...
      const vec = encodeGenome(g);
      const base = 23;
      expect(vec[base]).toBe(g.network.hiddenLayers[0].neurons / 512);
    });
    ...
    test('should set one-hot for connection type', () => {
      ...
      const vec = encodeGenome(g);
      const base = 23;
      const connTypes: ConnectionType[] = ['dense-skip', 'fully-connected', 'residual-connection'];
  ...
  describe('decodeGenome', () => {
    test('should throw on wrong vector length', () => {
      ...
      expect(() => decodeGenome(short, template)).toThrow('expected vector of length');
    });
    ...
    test('should roundtrip a default genome', () => {
      ...
      expect(decoded.rl.horizon.maxEpisodeLength).toBe(original.rl.horizon.maxEpisodeLength);
      expect(decoded.rl.horizon.nStepReturn).toBe(original.rl.horizon.nStepReturn);
    ...
    test('should preserve template identity fields (id, fitness, gaControl)', () => {
      ...
      const decoded = decodeGenome(vec, modded);
    ...
    test('should recover layer structure (count, neurons, activations)', () => {
      ...
      expect(decoded.network.hiddenLayers.length).toBe(original.network.hiddenLayers.length);
    ...
    test('should clamp extreme encoded values to valid ranges', () => {
      ...
      vec[0] = 10;
      vec[2] = 100; // clipMin  — decoded as-is for clamp, but enforced clipMin < clipMax in rewardShaping
      const decoded = decodeGenome(vec, original);
    ...
    test('should decode argmax for one-hot connection types', () => {
      ...
      vec[base + 1 + 8 + 2] = 1;
      const decoded = decodeGenome(vec, original);
      expect(decoded.network.hiddenLayers[0].connectionType).toBe('residual-connection');
    });
  ...
  describe('roundtrip — extreme RL hyperparameters', () => {
    test('should roundtrip extreme gamma edges', () => {
      ...
      let dec = decodeGenome(vec, g);
      expect(dec.rl.gamma).toBeCloseTo(0.8, 4);

      g.rl.gamma = 0.9999;
      vec = encodeGenome(g);
      dec = decodeGenome(vec, g);
  ...
  describe('encodePopulation / decodePopulation', () => {
    test('should encode and decode back to identical genome array', () => {
      const pop = [createDefaultGenome('a'), createDefaultGenome('b'), createDefaultGenome('c')];
