...
describe('Loss Functions', () => {
  ...
  describe('root-mean-squared-error', () => {
    ...
    test('gradient should be finite', () => {
      const grad = LOSSES['root-mean-squared-error'].gradient(output, target, cfg);
      for (const g of grad) expect(Number.isFinite(g)).toBe(true);
    });
  });
  ...
  describe('Kullback-Leibler-divergence', () => {
    ...
    test('gradient should be finite', () => {
      const out = makeOutput([0.1, 0.8, 0.1]);
      const tgt = makeTarget([0.2, 0.6, 0.2]);
      const grad = LOSSES['Kullback-Leibler-divergence'].gradient(out, tgt, cfg);
      for (const g of grad) expect(Number.isFinite(g)).toBe(true);
    });
  });
});
