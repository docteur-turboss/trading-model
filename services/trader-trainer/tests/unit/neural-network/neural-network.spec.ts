...
describe('NeuralNetwork', () => {
  describe('constructor', () => {
    ...
    it('should throw when fewer than 2 layers are provided', () => {
      expect(() => new NeuralNetwork(makeConfig({ neuronsByLayer: [4] }))).toThrow();
    });

    it('should throw when a layer size is zero', () => {
      expect(() => new NeuralNetwork(makeConfig({ neuronsByLayer: [4, 0, 3] }))).toThrow();
    });

    it('should throw when a layer size is negative', () => {
      expect(() => new NeuralNetwork(makeConfig({ neuronsByLayer: [4, -2, 3] }))).toThrow();
    });

    it('should throw when activationType length does not match layer count', () => {
      expect(
        () => new NeuralNetwork(makeConfig({ activationType: ['ReLu', 'tanh', 'sigmoid'] }))
      ).toThrow();
    });
  ...
  describe('train', () => {
    ...
    it('should throw when input size mismatches', () => {
      ...
      expect(() => nn.train(new Float32Array([1, 2]), new Float32Array([1, 0, 0]))).toThrow();
    });
