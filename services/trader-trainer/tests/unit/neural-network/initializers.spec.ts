...
describe('Initializers', () => {
  test('zeros should return 0', () => {
    expect(INITIALIZERS.zeros.initialize(5, 3)).toBe(0);
  });

  test('he should return a finite number', () => {
    const result = INITIALIZERS.he.initialize(10, 5);
    expect(Number.isFinite(result)).toBe(true);
  });

  test('xavier should return a number in [-limit, limit]', () => {
    for (let i = 0; i < 100; i++) {
      const result = INITIALIZERS.xavier.initialize(10, 5);
  ...
  test('leCun should return a finite number', () => {
    const result = INITIALIZERS.leCun.initialize(10, 5);
    expect(Number.isFinite(result)).toBe(true);
  });

  test('random should return a number in [-1, 1]', () => {
    for (let i = 0; i < 100; i++) {
      const result = INITIALIZERS.random.initialize(0, 0);
      expect(result).toBeGreaterThanOrEqual(-1);
      expect(result).toBeLessThanOrEqual(1);
    }
  });
