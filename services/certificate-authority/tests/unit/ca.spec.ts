...
function createCa() {
  return new CertificateAuthority({
    ...
    certificateStore: mockCertStore as any,
    crlStore: mockCrlStore as any,
    caStore: mockCaStore as any,
  });
}
...
describe('CertificateAuthority', () => {
  ...
  describe('initialize', () => {
    it('should bootstrap CA when no key file exists', async () => {
      ...
      mockCaStore.save.mockResolvedValue(undefined);
      ...
      expect(mockCaStore.save).toHaveBeenCalled();
    ...
    it('should load existing CA key and cert from store', async () => {
      ...
      expect(mockReadFileSync).toHaveBeenCalledWith('/etc/ca-keys/ca-key.pem', 'utf8');
  ...
  describe('signServiceCertificate', () => {
    ...
    it('should sign a certificate and save to store', async () => {
      ...
      expect(mockCertStore.save).toHaveBeenCalled();
  ...
  describe('revokeCertificate', () => {
    it('should throw if certificate not found', async () => {
      ...
      await expect(ca.revokeCertificate('SN-MISSING', 'key_compromise')).rejects.toThrow(
        'Certificate SN-MISSING not found'
      );
      expect(mockCrlStore.add).not.toHaveBeenCalled();
    });
    ...
    it('should add revoked cert to CRL store', async () => {
      ...
      expect(mockCrlStore.add).toHaveBeenCalledWith(
  ...
  describe('getCrl', () => {
    it('should return all revoked certificates', async () => {
      ...
      mockCrlStore.getAll.mockResolvedValue([
        { serialNumber: 'SN-REVOKED', serviceId: 'svc-1', revokedAt: new Date(), reason: 'test' },
      ]);
