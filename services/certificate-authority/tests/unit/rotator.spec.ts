...
describe("Rotator", () => {
	...
	describe("rotation logic", () => {
		it("should fetch expiring certificates on each interval", async () => {
			MOCK_CERTIFICATE_STORE.getExpiring.mockResolvedValue([]);

			rotator.start();
			jest.advanceTimersByTime(86400000);
			await jest.advanceTimersByTimeAsync(0);

			expect(MOCK_CERTIFICATE_STORE.getExpiring).toHaveBeenCalledWith(17280000);
		});
		...
		it("should log error when rotation fails", async () => {
			MOCK_CERTIFICATE_STORE.getExpiring.mockRejectedValue(
				new Error("DB error")
			);
