...
import type { LockConnectionManager } from "./lock-connection-manager";
...
export class LockAcquisitionChain {
	...
	async release(context: LockContext, savedToken: number): Promise<void> {
		if (
			await this._releaseOnBackend(
				this._connectionManager.mongoBackend,
				context,
				savedToken
			)
		) {
		...
		if (await this._releaseOnBackend(this._redisBackend, context, savedToken)) {
			return;
		}
		await this._filesystemBackend.release(context, savedToken);
	}

	private async _tryMongoAcquire(
		context: LockContext,
		ttlMs: number
	): Promise<number | null> {
	...
	private async _tryRedisAcquire(
		context: LockContext,
		ttlMs: number,
		redisUrl?: string
	): Promise<number | null> {
		const backend = redisUrl
			? new RedisLockBackend(redisUrl)
			: this._redisBackend;
		return backend.acquire(context, ttlMs);
	}

	private async _releaseOnBackend(
		backend: LockBackend,
		context: LockContext,
		token: number
	): Promise<boolean> {
