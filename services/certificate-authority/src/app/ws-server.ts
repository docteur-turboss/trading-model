...
const WsSignSchema = z.object({
  type: z.literal('sign'),
  id: z.string().min(1),
  data: z.object({
    serviceId: z.string().min(1),
    csr: z.string().min(1),
    ttlMs: z.number().positive().max(env.CERT_MAX_TTL_MS).optional(),
  }),
});
...
function checkUnauthRateLimit(clientIdentity: string): boolean {
  const now = Date.now();
  const entry = unauthSignAttempts.get(clientIdentity);
  if (!entry || now > entry.resetAt) {
    unauthSignAttempts.set(clientIdentity, { count: 1, resetAt: now + UNAUTH_WINDOW_MS });
    return true;
  }
...
export function attachWsServer(httpsServer: https.Server): WebSocketServer {
  ...
  wss.on('connection', (ws: WebSocket, req) => {
    ...
    logger.info('WSS client connected to CA (awaiting auth)', { clientIdentity });
