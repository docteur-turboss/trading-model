...
interface CertStore {
  getBySerial(serialNumber: string): Promise<{ certPem: string; serviceId: string } | null>;
}
...
export class CertRenewalService {
  private readonly popVerifier = new PopVerifier();

  constructor(
    private readonly certStore: CertStore,
  ...
  async renew(
    ...
    const oldCert = await this.certStore.getBySerial(oldSerialNumber);
    if (!oldCert) {
      throw new CertRenewalError('Original certificate not found', 404);
    }

    // 3. Verify proof-of-possession (client still holds the private key)
    if (!this.popVerifier.verify(oldCert.certPem, nonce, signature)) {
    ...
    if (this.lock) {
      ...
      try {
        return await this.ca.signServiceCertificate(serviceId, csr);
      } finally {
        await this.lock.release();
      }
    }

    return this.ca.signServiceCertificate(serviceId, csr);
  }
}
