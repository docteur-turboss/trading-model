...
async function request<T>(method: string, path: string, body?: unknown): Promise<T> {
  ...
  if (!res.ok) {
    ...
    throw new ApiError(res.status, (err as { error?: string }).error ?? 'Unknown error');
  }
...
export const api = {
  ...
  getAuditEvents: (params?: AuditFilter) =>
    request<PaginatedEvents>('GET', `/audit/events${toQuery(params as Record<string, unknown>)}`),

  getJobs: () => request<JobList>('GET', '/jobs'),
  getJobDetail: (id: string) => request<JobDetail>('GET', `/jobs/${id}`),
  cancelJob: (id: string) => request<void>('PATCH', `/jobs/${id}/status`, { status: 'cancelled' }),

  getDlqMessages: () => request<DlqMessageList>('GET', '/messages/dlq'),
  purgeDlq: () => request<void>('DELETE', '/messages/dlq'),
  retryDlqMessage: (id: string) => request<void>('POST', `/messages/dlq/${id}/retry`),
  ...
  getCandles: (symbol: string, interval: string) =>
    request<Candle[]>('GET', `/scraper/candles?symbol=${symbol}&interval=${interval}`),
  getTickers: (symbol: string) => request<Ticker>('GET', `/scraper/tickers/${symbol}`),
  ...
  getConfig: (service?: string) =>
    request<ConfigEntry[]>('GET', `/discovery/config${service ? `/${service}` : ''}`),

  getCertificates: () => request<CertificateEntry[]>('GET', '/ca/certificates'),
  revokeCertificate: (id: string) => request<void>('POST', '/ca/revoke', { certificateId: id }),
