...
function createMockResponse(
  ...
  const mockReq = {
    ...
    write: jest.fn(),
      ...
      if (events.data) {
        for (const h of events.data) h(Buffer.from(body));
      }
      if (events.end) {
...
describe('proxy-handler', () => {
  ...
  it('should forward a GET request and return the response', async () => {
    ...
    https.request = createMockHttps((_opts: any, callback: (res: any) => void) => {
      ...
      if (events.data) {
        for (const h of events.data) h(Buffer.from(JSON.stringify({ data: 'ok' })));
      }
      if (events.end) {
        for (const h of events.end) h();
      }
  ...
  it('should strip x-api-key header from forwarded request', async () => {
    ...
    await forward(req, mockTarget, '/test');
    ...
    https.request = jest.fn().mockImplementation(() => {
      const timeoutReq = {
        on: jest.fn((event: string, handler: () => void) => {
          if (event === 'timeout') timeoutEvents.push(handler);
          return timeoutReq;
        }),
        write: jest.fn(),
        end: jest.fn().mockImplementation(() => {
          for (const h of timeoutEvents) h();
        }),
        destroy: jest.fn(),
      };
      return timeoutReq;
  ...
  it('should forward POST body', async () => {
    ...
    https.request = createMockHttps((_opts: any, callback: (res: any) => void) => {
      ...
      if (events.data) {
        for (const h of events.data) h(Buffer.from(JSON.stringify({ saved: true })));
      }
      if (events.end) {
        for (const h of events.end) h();
      }
    ...
    const result = await forward(req, mockTarget, '/v1/api/create');
