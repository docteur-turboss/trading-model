...
import { insertOrderBook, selectOrderBookBy } from '../../src/infra/market-data/schema/orderBook.schema'

const buildOrderBook = (symbol: string, ts: Date) => ({
  symbol,
  source: 'binance',
  market: 'crypto' as const,
  bids: [{ price: 50000, quantity: 0.5 }, { price: 49990, quantity: 1.0 }],
  asks: [{ price: 50010, quantity: 1.0 }, { price: 50020, quantity: 0.8 }],
  timestamp: ts,
})

const buildOrderBookAltSource = (symbol: string, ts: Date) => ({
  symbol,
  source: 'kraken',
  market: 'crypto' as const,
...
describe('OrderBook storage — full insert/query integration', () => {
  ...
  beforeEach(async () => {
    await insertOrderBook([buildOrderBook('BTCUSDT', t1) as never])
    await insertOrderBook([buildOrderBook('BTCUSDT', t2) as never])
    await insertOrderBook([buildOrderBook('ETHUSDT', t3) as never])
    await insertOrderBook([buildOrderBookAltSource('BTCUSDT', t1) as never])
  })
  ...
  it('should query entries after a given timestamp', async () => {
    ...
    for (const entry of after!) {
      expect(new Date(entry!.timestamp).getTime()).toBeGreaterThan(t1.getTime())
    }
  })
  ...
  it('should return results sorted by timestamp ascending for after query', async () => {
    const after = await selectOrderBookBy.timestamp.after(t1.getTime() - 3600000)
    for (let i = 1; i < after!.length; i++) {
      expect(new Date(after![i]!.timestamp).getTime())
        .toBeGreaterThanOrEqual(new Date(after![i - 1]!.timestamp).getTime())
    }
  })
