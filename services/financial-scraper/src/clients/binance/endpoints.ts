export const BINANCE_ENDPOINTS = {
   ...
   * @returns
   */
  compressedAggregateTrades: (symbol?: string, fromId?: string | number, limit?: number): string =>
    symbol && fromId && limit
  ...
  change24hrStats: (symbol?: string[]): string =>
    symbol && symbol.length > 0
      ? `/api/v3/ticker/24hr?symbols=${encodeURIComponent(`[${symbol.map(e => `"${e}"`)}]`)}`
      : `/api/v3/ticker/24hr`,
  /**
  ...
  TradingDayTicker: (symbol?: string[]): string =>
    symbol && symbol.length > 0
      ? `/api/v3/ticker/tradingDay?symbols=${encodeURIComponent(`[${symbol.map(e => `"${e}"`)}]`)}`
  ...
  symbolPriceTicker: (symbol?: string[]): string =>
    symbol && symbol.length > 0
      ? `/api/v3/ticker/price?symbols=${encodeURIComponent(`[${symbol.map(e => `"${e}"`)}]`)}`
      : `/api/v3/ticker/price`,
  /**
   * Order book ticker.
   * @param symbols {string[]} - list of symbols to fetch stats for
  ...
  orderBookTicker: (symbol?: string[]): string =>
    symbol && symbol.length > 0
      ? `/api/v3/ticker/bookTicker?symbols=${encodeURIComponent(`[${symbol.map(e => `"${e}"`)}]`)}`
