...
export const LOSSES: Record<LossFunctionType, LossDefinition> = {
  ...
  "root-mean-squared-error": {
    ...
    gradient(output, target) {
      ...
      const rmse =
        Math.sqrt(sum * invN) + EPSILON;

      const scale = invN / rmse;

      for (let i = 0; i < n; i++) {
        out[i] =
          (output[i] - target[i]) * scale;
      }
  ...
  "binary-cross-entropy": {
    loss(output, target) {
      ...
      for (let i = 0; i < n; i++) {
        ...
        sum -=
          t * Math.log(o + EPSILON) +
          (1 - t) *
            Math.log(1 - o + EPSILON);
      }
