...
// ----------------------------------------------------------------
// Valid enum sets (mirrors genome_types literals)
// ----------------------------------------------------------------
...
export function validateGenome(g: Genome): ValidationResult {
  ...
  if (!Array.isArray(g.network.hiddenLayers) || g.network.hiddenLayers.length === 0) {
    ...
    g.network.hiddenLayers.forEach((l, i) => validateLayer(errors, `network.hiddenLayers[${i}]`, l));
  }
  ...
  if (rs.clipMin >= rs.clipMax) {
    err(errors, "rl.rewardShaping.clip", "clipMin must be < clipMax", { clipMin: rs.clipMin, clipMax: rs.clipMax });
  }
  ...
  if (cp.clipMin >= cp.clipMax) {
    err(errors, "rl.continuousPolicy.clip", "clipMin must be < clipMax", { clipMin: cp.clipMin, clipMax: cp.clipMax });
  }
...
export function repairGenome(g: Genome): Genome {
  ...
  const network = {
    ...
    hiddenLayers,
    normalization: VALID_NORM_TYPES.has(g.network.normalization)
