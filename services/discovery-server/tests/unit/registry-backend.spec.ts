...
describe('InMemoryRegistryBackend', () => {
  ...
  it('should filter by region via getInstances', async () => {
    const backend = new InMemoryRegistryBackend();
    await backend.registerInstance(makeInstance({ instanceId: 'i1', region: 'us-east-1' }));
    await backend.registerInstance(makeInstance({ instanceId: 'i2', region: 'eu-west-1' }));
  ...
  describe('updateHeartbeat', () => {
    ...
    it('should return false for unknown service', async () => {
      const backend = new InMemoryRegistryBackend();
      const result = await backend.updateHeartbeat('unknown-service', 'test-instance-1');
    ...
    it('should return false for unknown instance', async () => {
      const backend = new InMemoryRegistryBackend();
      await backend.registerInstance(makeInstance());
      const result = await backend.updateHeartbeat('financial-scraper-service', 'unknown-instance');
      expect(result).toBe(false);
    });
  });
  ...
  describe('getInstance', () => {
    it('should return a specific instance', async () => {
      const backend = new InMemoryRegistryBackend();
      await backend.registerInstance(makeInstance());
      const instance = await backend.getInstance('financial-scraper-service', 'test-instance-1');
    ...
    it('should return undefined for unknown service', async () => {
      const backend = new InMemoryRegistryBackend();
      const instance = await backend.getInstance('unknown-service', 'test-instance-1');
    ...
    it('should return undefined for unknown instance', async () => {
      const backend = new InMemoryRegistryBackend();
      await backend.registerInstance(makeInstance());
      const instance = await backend.getInstance('financial-scraper-service', 'unknown-instance');
      expect(instance).toBeUndefined();
    });
  });
  ...
  describe('removeInstance', () => {
    ...
    it('should remove the service map when last instance is removed', async () => {
      const backend = new InMemoryRegistryBackend();
      await backend.registerInstance(makeInstance());
      await backend.removeInstance('financial-scraper-service', 'test-instance-1');
    ...
    it('should return false for unknown service', async () => {
      const backend = new InMemoryRegistryBackend();
      const result = await backend.removeInstance('unknown-service', 'test-instance-1');
      expect(result).toBe(false);
    });
    ...
    it('should return false for unknown instance', async () => {
      const backend = new InMemoryRegistryBackend();
      await backend.registerInstance(makeInstance());
      const result = await backend.removeInstance('financial-scraper-service', 'unknown-instance');
      expect(result).toBe(false);
    });
  });
  ...
  describe('generateInstanceId', () => {
    it('should return a non-empty base64 string', () => {
      const backend = new InMemoryRegistryBackend();
      const id = backend.generateInstanceId('financial-scraper-service', '192.168.1.10', 8444);
  ...
  describe('validInstanceToken', () => {
    ...
    it('should return false for token with wrong part count', async () => {
      const backend = new InMemoryRegistryBackend();
      const result = await backend.validInstanceToken('invalid-token', 'test-instance-1');
      expect(result).toBe(false);
    });
  ...
  describe('verifyInstanceName', () => {
    ...
    it('should return false for unknown service names', () => {
      const backend = new InMemoryRegistryBackend();
      expect(backend.verifyInstanceName('completely-made-up-service')).toBe(false);
    });
  });
