...
import { validServiceInstance, secondServiceInstance, otherServiceInstance } from '../fixtures/index'
...
describe('ServiceRegistry', () => {
  ...
  describe('registerInstance', () => {
    ...
    it('should update an existing instance on re-registration', () => {
      ...
      const updated = registry.registerInstance(validServiceInstance({ port: 9999 }))
      expect(updated.instanceId).toBe('test-instance-1')
      expect(updated.port).toBe(9999)
      expect(updated.lastHeartbeat).toBeGreaterThanOrEqual(first.lastHeartbeat!)
    })

    it('should update lastHeartbeat on re-registration', () => {
      const first = registry.registerInstance(validServiceInstance({ lastHeartbeat: Date.now() - 5000 }))
      const updated = registry.registerInstance(validServiceInstance({ lastHeartbeat: Date.now() }))
      expect(updated.lastHeartbeat).toBeGreaterThanOrEqual(first.lastHeartbeat!)
    })
  })
  ...
  describe('updateHeartbeat', () => {
    ...
    it('should return false for unknown service', () => {
      const result = registry.updateHeartbeat('unknown-service', 'test-instance-1')
  ...
  describe('getInstance', () => {
    ...
    it('should return undefined for unknown service', () => {
      const instance = registry.getInstance('unknown-service', 'test-instance-1')
  ...
  describe('removeInstance', () => {
    ...
    it('should return false for unknown service', () => {
      const removed = registry.removeInstance('unknown-service', 'test-instance-1')
  ...
  describe('validInstanceToken', () => {
    it('should return true for a valid token', () => {
      const registered = registry.registerInstance(validServiceInstance())
      const isValid = registry.validInstanceToken(registered.token as string, registered.instanceId!)
