...
export class MessageStore implements IMessageRouting, IMessageStorage, IWalLifecycle {
	...
	async claimPendingMessages(groupName: string, consumerId: string, minIdleMs = 60_000, count = 100): Promise<number> { return this._routingFacade.claimPendingMessages(groupName, consumerId, minIdleMs, count); }
	...
	async addPendingAck(instanceId: string, messageId: string, data: { topic: string; subscriberUrl: string; message: Message }): Promise<void> { return this._routingFacade.addPendingAck(instanceId, messageId, data); }
	...
	async getPendingAcks(instanceId: string): Promise<Record<string, { topic: string; subscriberUrl: string; message: Message }>> { return this._routingFacade.getPendingAcks(instanceId); }
