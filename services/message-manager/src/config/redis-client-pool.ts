...
export class RedisClientPool {
	...
	async getOrCreate(buildClient: () => Redis): Promise<Redis> {
		...
		return this._connectionDeduper.run(() =>
			this._createAndConnectClient(buildClient)
		);
	}
	getConnection(buildClient: () => Redis): Promise<Redis> {
		return this.getOrCreate(buildClient);
	}
	destroyAll(): void {
		if (this._client) {
			this._destroyClient(this._client);
