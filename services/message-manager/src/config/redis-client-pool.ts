...
export class RedisClientPool {
	...
	private _promise: Promise<Redis> | null = null;
	...
	private async _waitForReconnect(): Promise<Redis | null> {
		...
		try {
			return await this._waitForClientReady();
		} catch {
			return null;
		}
	}

	private async _resolvePendingPromise(): Promise<Redis | null> {
		return this._promise === null ? null : await this._promise;
	}

	private _startNewConnection(
		buildClient: () => Redis
	): Promise<Redis> {
		this._promise = this._createAndConnectClient(buildClient).finally(
			() => {
				this._promise = null;
			}
		);
	...
	private async _createAndConnectClient(
		buildClient: () => Redis
	): Promise<Redis> {
		...
		try {
			...
			if (!this._isClosed()) {
				logger.error(`${this._name}: failed to connect`, {
	...
	private _replaceClient(client: Redis): void {
		if (this._client && this._client !== client) {
			this._destroyClient(this._client);
		}
		this._client = client;
	}
	...
	async getOrCreate(buildClient: () => Redis): Promise<Redis> {
		...
		const existing = await this._resolvePendingPromise();
		if (existing) {
			return existing;
		}
		...
		return this._startNewConnection(buildClient);
	}
