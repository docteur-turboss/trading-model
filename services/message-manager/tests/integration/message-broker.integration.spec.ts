...
describe("Message Broker Integration", () => {
  ...
  it("should deliver a published message to a subscribed service", async () => {
    ...
    const [targetUrl, body] = httpClient.post.mock.calls[0] as [string, { message: { payload: { price: number; symbol: string } } }];
  ...
  it("should generate unique message IDs for each publish", async () => {
    ...
    const id1 = (httpClient.post.mock.calls[0][1] as { message: { metadata: { messageId: string } } }).message.metadata.messageId;
    const id2 = (httpClient.post.mock.calls[1][1] as { message: { metadata: { messageId: string } } }).message.metadata.messageId;
