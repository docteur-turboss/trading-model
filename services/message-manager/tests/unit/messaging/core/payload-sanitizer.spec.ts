...
describe('sanitizePayload', () => {
  ...
  it('should throw on MongoDB operator keys', () => {
    expect(() => sanitizePayload({ $where: 'malicious' })).toThrow('Blocked operator');
    expect(() => sanitizePayload({ $regex: '.*' })).toThrow('Blocked operator');
    expect(() => sanitizePayload({ nested: { $ne: 1 } })).toThrow('Blocked operator');
  });
  ...
  it('should throw when exceeding maximum nesting depth', () => {
    ...
    expect(() => sanitizePayload(deep)).toThrow('Payload exceeds maximum nesting depth');
  });
