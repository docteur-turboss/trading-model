...
describe('Broker', () => {
  ...
  describe('publish', () => {
    ...
    it('should generate a unique messageId for each publish', async () => {
      ...
      const id1 = (mockDispatcher.dispatch.mock.calls[0][0] as { metadata: { messageId: string } })
        .metadata.messageId;
      const id2 = (mockDispatcher.dispatch.mock.calls[1][0] as { metadata: { messageId: string } })
        .metadata.messageId;
    ...
    it('should set emittedAt to current time', async () => {
      ...
      const emittedAt = (
        mockDispatcher.dispatch.mock.calls[0][0] as { metadata: { emittedAt: Date } }
      ).metadata.emittedAt.getTime();
