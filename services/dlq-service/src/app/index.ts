...
import { initializeTelemetry, shutdownTelemetry } from '@trading-model/common/server/telemetry';
import { normalizeError } from '@trading-model/common/utils/errors';

import { createServer } from './server';
import { AddressManager } from '../config/address-manager';
import { getDb, closeDb, resetDbState } from '../config/db';
...
async function ensureRedisQueue(): Promise<void> {
  try {
    ...
    logger.warn('Redis queue unavailable on start — operations continue in DEGRADED mode', {
...
function createResilientTlsBootstrap(): TlsBootstrapOptions | null {
  try {
    ...
    logger.warn('TLS bootstrap from CA unavailable — falling back to file-based TLS config', {
      error: normalizeError(err),
    });
    return null;
  }
...
createBootstrap({
  ...
  onStart: () => {
    ...
    AddressManager.start().catch(err => {
      logger.warn('Address-manager start failed (non-fatal, env var fallback used)', {
        error: normalizeError(err),
      });
    });
    ensureRedisQueue();
